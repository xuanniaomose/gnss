...
class WebAppInterface(private val fragmentRef: WeakReference<MapFragment?>) {
    ...
    fun getCurrentLocation(currentLocation: Location): String {
        if (currentLocation != null) {
            try {
                val json = JSONObject()
                json.put("lat", currentLocation.latitude)
                json.put("lng", currentLocation.longitude)
                json.put("accuracy", currentLocation.accuracy)
                json.put("speed", currentLocation.speed)
                json.put("bearing", currentLocation.bearing)
                json.put("time", currentLocation.time)
                json.put("provider", currentLocation.provider)
